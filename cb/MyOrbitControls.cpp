...
struct MyOrbitControls::Impl {

    OpenGLCanvas& canvas;
    MyOrbitControls& scope;
    Camera& camera;
    ...
    Impl(MyOrbitControls& scope, OpenGLCanvas& canvas, Camera& camera)
    ...
    void pan(float deltaX, float deltaY) {
        ...
        if (auto perspective = camera.as<PerspectiveCamera>()) {
            ...
            std::cerr << "[MyOrbitControls] encountered an unknown camera type - pan disabled." << std::endl;
    ...
    struct MyKeyListener: KeyListener {
        ...
        MyOrbitControls& controls;

        explicit MyKeyListener(MyOrbitControls& controls)
            : controls(controls) {}
    ...
    struct MyMouseMoveListener: MouseListener {
        ...
        explicit MyMouseMoveListener(MyOrbitControls& controls): controls(controls) {}
    ...
    struct MyMouseUpListener: MouseListener {
        ...
        MyMouseUpListener(MyOrbitControls& scope, MyMouseMoveListener* mouseMoveListener)
    ...
    struct MyMouseListener: MouseListener {
        ...
        explicit MyMouseListener(MyOrbitControls& scope)
...
MyOrbitControls::MyOrbitControls(Camera& camera, OpenGLCanvas& canvas)
    : pimpl_(std::make_unique<Impl>(*this, canvas, camera)) {}


bool MyOrbitControls::update() {

    return pimpl_->update();
}

float MyOrbitControls::getAutoRotationAngle() const {

    return 2 * math::PI / 60 / 60 * this->autoRotateSpeed;
}

float MyOrbitControls::getZoomScale() const {

    return std::pow(0.95f, this->zoomSpeed);
}

MyOrbitControls::~MyOrbitControls() = default;
